...
export const middleware = async (req) => {
    ...
    // 보호할 경로를 배열에 정의합니다.
    const protectedPaths = ['/dash', '/user', '/ticket', '/teacher', '/class', '/schedule', '/notice'];

    // 오류를 가정하고 메시지를 설정합니다.
    if (!req.cookies.get('next-auth.session-token') && protectedPaths.some((path) => pathname.startsWith(path))) {
